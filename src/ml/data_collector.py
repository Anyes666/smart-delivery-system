...
class TravelTimeDataCollector:
    ...
    def __init__(self, auto_save: bool = True, save_interval: int = 20):
        ...
        self._adaptive_trained_count: Dict[str, int] = {}  # 已用于自适应训练的样本数
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        atexit.register(self._flush)  # 崩溃时也保证刷盘
        self.real_time_mode = True   # 默认真实时间; --fast 模式设为 False

    def set_real_time_mode(self, mode: bool):
        """设置采集模式: True=真实时间(默认), False=模拟/快速."""
    ...
    def record(
        ...
        sample = {
            ...
            "timestamp": ts,
            "city": city,
            "real_time": real_time,  # True=真实时间采集, False=模拟时间
        }
